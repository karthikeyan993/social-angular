...
export class AuthService {
  ...
  signup(userData:any){
    return this.http.post<AuthData>(`https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=${environment.firebaseWebAPIKey}`,{
      ...
      this.handleAuthentication(userData.fname, userData.lname, userData.imageUrl, resData.email, resData.localId, resData.idToken, +resData.expiresIn);
    }))
  }
  ...
  login(email: string, password:string){
    return this.http.post<AuthData>(`https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=${environment.firebaseWebAPIKey}`,{
      ...
      this.handleAuthenticationLogIn(resData.email, resData.localId, resData.idToken, +resData.expiresIn);
    }));
  }
  ...
  private handleAuthentication(
    ...
    const user = new User(fname, lname, imageUrl, email, userId, token, expirationDate);
    this.user.next(user);
  ...
  private handleAuthenticationLogIn(email: string, userId: string, token: string, expiresIn: number) {
    this.http.get<any>(`https://social-angular-76383-default-rtdb.asia-southeast1.firebasedatabase.app/users.json?orderBy="email"&equalTo="${email}"`)
      .subscribe(userData => {
        ...
        const user = new User(firstName, lastName, imageUrl, email, userId, token, expirationDate);
